...
/* 설명.
*   자바빈(Java Bean) 이란?
*    JSP에서 사용되는 표준 액션 태그로 접근할 수 잇게 만든 자바 클래스 형태
...
*   2. 필드의 접근 제어자는 private 으로 선언해야 함.(캡슐화 적용)
*   3. 기본 생성자가 명시적으로 존재해야 한다. (매개변수 있는 생성자는 선택 사항)
*   4. 모든 필드에 접근 가능한 설정자(setter)과 접근자(getter)가 public 으로 작성되어 있어야 함.
*   5. 직렬화(Serializable 구현)을 고려해야 함.(선택 사항)
* */
