...
public class Application {
    public static void main(String[] args){
        ...
        /* 목차. 1. 기본 자료형(primitive) 을 매개변수로 전달받는 메소드 호출 */
        /* 설명.  call by value
            리터럴 값(참조 주솟값x) 을 전달해서 메소드를 호출 시에는
            서로 다른 지역변수로 서로 영향 x */
        int num = 20;
        ...
        pt.testPrimitiveTypeParameter(rectangle);
        pt.testPrimitiveTypeParameter(ref);
        ...
        /* 목차. 5. 가변 인자를 매개변수로 전달받는 메소드 호출(자바는 웬만하면 사용을 권장하지 않음.) */
        pt.testVariableLengthArrayParamter();
        pt.testVariableLengthArrayParamter("홍길동");
        pt.testVariableLengthArrayParamter("유관순", "볼링");
        pt.testVariableLengthArrayParmeter(new String[]{"강감찬", "낚시", "독서"});
