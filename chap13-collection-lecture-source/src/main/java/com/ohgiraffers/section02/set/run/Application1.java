...
public class Application1 {
    public static void main(String[] args) {
        ...
        /* 수업목표. Set(집합) 자료구조의 특성을 이해하고 hashSet을 이용할 수 있다.  */

//        HashSet<String> hSet = new HashSet<>();
        Set<String> hSet = new HashSet<>();
        hSet.add(new String("java"));
        hSet.add(new String("java"));   // 안 들어감. -> 다른 객체로 인식해서 넣으려면? 오버라이딩 해줘야 함?
        hSet.add(new String("mariaDB"));
        hSet.add(new String("servlet"));
        hSet.add(new String("html"));
        hSet.add(new String("spring"));
        // 내부적인 알고리즘이 존재 -> HashMap 알고리즘(?)에 의해 정렬됨
        // 실행해도 매번 같은 순서로 보여줌 **** 해시셋이어서???
        // 순서 보장 x : 넣은 순서대로 저장되지 않음
